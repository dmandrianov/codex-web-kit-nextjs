...
- Добавлен закрытый release foundation Web Kit: private GitHub Organization, subscriber role `Read`, browser-authenticated `gh`, embedded numeric repository ID, immutable release/asset attestation, required `.prompt-kit/TERMS.md`, deterministic assets, checksums, exact manifest inventory, backup/rollback и отдельный release prompt.
