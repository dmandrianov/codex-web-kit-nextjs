# Собрать и проверить закрытый Prompt Kit Release

## Когда использовать

Только в source repository Web Kit, когда maintainer просит подготовить локальный release candidate, проверить release artifacts или — отдельной явной командой — выполнить конкретные Git/GitHub publication actions в private GitHub Organization repository.
...
Ты действуешь как release engineer, reproducible-build reviewer и осторожный maintainer закрытого подписочного Prompt Kit.

## Цель

Собрать локально проверяемые stable release assets `web-kit-vX.Y.Z.zip`, `web-kit-vX.Y.Z.tar.gz` и `SHA256SUMS`, создать manifest v1 с private repository identity, включить закрытые условия, проверить payload и подготовить preflight report. Git/GitHub state-changing actions не входят в обычный build/verify pass и выполняются только после отдельного явного разрешения пользователя.
...
- Private Organization repository metadata: bootstrap/current full name, positive numeric repository ID, `private: true`, `owner.type: Organization`, immutable releases `enabled: true`.
...
- Не собирай publishable candidate без корневого `TERMS.md`, mapped required `.prompt-kit/TERMS.md`, positive numeric repository ID, full name private Organization repository и включённых immutable releases. Open-source `LICENSE` не является publication gate.
...
- Не запрашивай, не печатай и не сохраняй human GitHub tokens, private keys или другие secret values. Для локальной maintainer-работы используй существующую GitHub CLI session и не вызывай `gh auth token`. GitHub Actions publication использует только короткоживущий installation token отдельного repository-scoped GitHub App; Client ID хранится в Actions variable, private key — в Actions secret, а их значения нельзя выводить, сохранять в source/assets или передавать подписчикам. Встроенного `github.token` недостаточно для immutable-settings endpoint с `Administration: read`.
- Не помещай payment emails, GitHub usernames подписчиков или access ledger в source, manifest, release notes либо assets.
...
3. Проверь release metadata:
   ...
   - корневой `TERMS.md` существует, описывает закрытую подписочную модель и после сборки становится required `.prompt-kit/TERMS.md`;
   - условия разрешают использовать и изменять версии в собственных и клиентских проектах и продолжать использовать версии, скачанные во время активной подписки, но запрещают standalone redistribution/resale, credential и release-archive sharing.
4. Проверь source tree read-only:
   ...
   - private Organization repository identity: configured full name, positive numeric ID, `private: true`, owner типа `Organization`;
...
5. Определи candidate status до сборки:
   - `publishable candidate`, если source version согласована, Git repository уже существует, worktree clean, commit известен, private Organization identity подтверждена, immutable releases включены и TERMS готовы;
...
6. Собери curated payload только из release allowlist:
   ...
   - closed-use terms, mapped из source `TERMS.md` в required `.prompt-kit/TERMS.md`; отсутствие TERMS блокирует publishable candidate;
...
7. Сгенерируй manifest v1:
   ...
   - `source.repositoryFullName`, positive numeric `source.repositoryId`, private Organization `source.transport`, tag и revision;
...
15. Если publish actions явно разрешены, перед каждой группой действий повторно проверь:
    ...
    - configured repository существует, остаётся private, принадлежит Organization и имеет numeric ID, совпадающий с embedded updater/config/manifest trust anchor;
...
- Changelog entry:
- Migration entry:
- Closed terms source/mapped file:
- Compatibility:
...
## Subscriber distribution boundary

- Outside collaborators use `Read`: configured/manual check
- Payment email and GitHub username kept outside repository: yes/no
- Subscriber access ledger included in source/assets: no
...
- Cancelled subscriber keeps downloaded versions but loses future access: covered by TERMS yes/no
...
- Publishable candidate привязан к private GitHub Organization repository через embedded positive numeric ID, а repository immutable releases configuration подтверждена как `enabled: true`; null identity или выключенная immutability остаются blocker.
- Root `TERMS.md` включён как required `.prompt-kit/TERMS.md` и отражает закрытую подписочную модель.
...
- Human GitHub credential и subscriber access ledger не запрошены, не выведены и не попали в source/assets; managed CI credential, если применим, не раскрыт.
...
После явно разрешённой публикации проверь private GitHub Release, `immutable: true`, signed attestation и local asset provenance read-only, затем обнови preflight report. Следующий пользовательский тест — отдельной subscriber-учётной записью с outside collaborator role `Read` пройти browser-based `gh auth login`, проверить release/asset attestation, установить release в disposable project и сказать `обнови базу`. Затем отозвать test access и убедиться, что future update блокируется до записи, а уже скачанная версия остаётся рабочей.
