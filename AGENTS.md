...
12. Если пользователь просит новую SEO-статью, пост в блог, guide, comparison, listicle, review, pillar article или FAQ-материал под поисковый запрос, используй `prompts/_content/01-write-seo-article.md`: прочитай `prompts/_guidelines/seo-content-writer-integration.md`, затем установленный `seo-content-writer/SKILL.md` и его четыре локальных reference-файла полностью, проверь pinned SHA-256 и явно вызови `$seo-content-writer`. Для остальных чистых copy-задач без UI-creator pass используй короткий обязательный контракт и `Site copy fast pass` из `prompts/_knowledge/site-copy-quality.md`; полный check оставляй длинному, критичному или рискованному тексту. Для hero/offer/CTA обращайся к `prompts/_guidelines/landing-copy-formulas.md` как к diagnostic fallback, только если прямой fact-backed текст не складывается. Truth и неподтверждённые claims проверяй всегда.
...
- Для чистой copy-задачи, где нового UI render не будет, сразу используй короткий обязательный контракт и `Site copy fast pass` из `prompts/_knowledge/site-copy-quality.md`; формулы для hero/offer/CTA используй только как diagnostic fallback, если прямой fact-backed вариант не работает.
- Для новой SEO-статьи, поста в блог или другого длинного материала под поисковый запрос используй pinned external `$seo-content-writer` через `prompts/_content/01-write-seo-article.md`. Этот cross-cutting route не меняет основную стадию сайта.
...
- Не копируй, не редактируй и не vendor upstream `seo-content-writer` внутрь Prompt Kit. Kit хранит preserved tag, pinned commit, checksums и explicit article route; missing или mismatched skill блокирует article draft без silent fallback.
...
- До UI render утверди смысл, факты, claims и пользовательское действие; неподтверждённое пометь `needs confirmation`. Для обычного текста выполни `Site copy fast pass` внутри текущего шага; полный `Site copy check` выполняй для длинного, критичного или рискованного текста и на полном quality-аудите.
